...
/**
 * Generate PDF as base64 string (for payment receipts or API responses)
 * Returns the PDF content as base64 instead of downloading
 */
export const generateLoanPDFAsBase64 = async (loanData, options = {}) => {
  try {
    const settings = await pdfSettingsService.getSettings();
    ...
    // Payment info if included
    if (options.paymentAmount) {
      doc.setFont(undefined, 'bold');
      doc.text(`Payment Received: $${parseFloat(options.paymentAmount).toFixed(2)}`, margin, yPosition);
      yPosition += 4;
    }
    
    if (options.paymentDate) {
      doc.setFont(undefined, 'normal');
      doc.text(`Payment Date: ${options.paymentDate}`, margin, yPosition);
      yPosition += 4;
    }
    
    if (options.paymentMethod) {
      doc.text(`Payment Method: ${options.paymentMethod}`, margin, yPosition);
      yPosition += 6;
    } else {
      yPosition += 2;
    }
    ...
    doc.setFont(undefined, 'normal');
    doc.setFontSize(8);
    const remainingBalance = loanData.remaining_balance !== undefined ? loanData.remaining_balance : 0;
    doc.text('REMAINING BALANCE', margin + colWidths.item + colWidths.category + 5, yPosition);
    doc.text(`$${parseFloat(remainingBalance).toFixed(2)}`, pageWidth - margin - 40, yPosition);
    yPosition += 6;
    ...
    doc.text('TOTAL PAYABLE', margin + colWidths.item + colWidths.category + 5, yPosition);
    doc.text(`$${parseFloat(totalPayable).toFixed(2)}`, pageWidth - margin - 40, yPosition);
    yPosition += 8;
    ...
    // ===== RETURN AS BASE64 =====
    const pdfAsBase64 = doc.output('dataurlstring').split(',')[1];
